...
| `run_queue_72h.sh` | Ordered queue with time estimates | yes |

Put these next to your notebooks, so `datasets/`, `saved_models/`, `saved_np/`
and `results/` resolve the same way. The `.npy` cache filenames match, so the
arrays you built already get reused rather than rebuilt.

## Two things to fix before running

**The evaluation pipeline will silently drop every ablation model.** Cell 10's
`MODEL_PATTERN` allows nothing between `{pct}pct` and the `_fold{i}` or
`_ensemble` suffix, so `model_ISIC2018_BT-UNet_5pct_frozen_fold0.keras` fails to
match. `discover_models` skips non-matching files without raising, which means
you get a short results table rather than an error. Copy the pattern and
`parse_model_name` from `eval_pattern_patch.py` into that cell, and add
`"Variant"` wherever the pipeline groups or pivots. Without that second part,
`pivot_table(aggfunc="mean")` averages two variants of the same family and
fraction into one cell. Run `python eval_pattern_patch.py` to see the tests pass.
...
Then work through `run_queue_72h.sh`. Stages 1 and 2 carry the main result;
3 through 6 explain it; 7 and 8 are for leftover time.

```bash
bash run_queue_72h.sh ISIC2018 1
```
...
Run `python cost_model.py` for the full per-stage budget. It comes to about 32 GPU
hours for ISIC2018's full queue plus KDSB's main sweep, serialised on one card,
which is 44% of a 72-hour booking.
...
2.1 h. Pretraining cost is estimated rather than measured, since `PRETRAIN_FLG`
was False in all three of your runs; time one real pretrain and set
`PRETRAIN_100EP_HOURS` in `cost_model.py`.

...
# Layer names the decoder needs back from a pretrained encoder.
...
def evalResult(gt, pred, num_class=2, fast=False, quiet=False):
    ...
    if not fast:
        if not _HAS_HAUSDORFF:
            raise ImportError("hausdorff not installed; pass fast=True or pip install hausdorff")
...
def build_backbone(encoder_path, trainable=True):
...
def build_segmentation_model(keyname, encoder_path=None, freeze_encoder=False,
    ...
    if encoder_path is None:
        ...
        x, skips = encoder(s)
        x = bottleneck(x)
        x = decoder(x, skips, prefix=f"{keyname}_dec")
        outputs = output_layer(x, prefix=f"{keyname}_out")
        model = Model(inputs, outputs, name=keyname)
    else:
        backbone = build_backbone(encoder_path, trainable=not freeze_encoder)
        skips = [backbone.get_layer(n).output for n in SKIP_LAYER_NAMES]
        x = decoder(backbone.output, skips, prefix=f"{keyname}_dec")
        outputs = output_layer(x, prefix=f"{keyname}_out")
